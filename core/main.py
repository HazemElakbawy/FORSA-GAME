...
CAR_WIDTH = 50
CAR_LENGTH = 25
CAR_SPEED = 0.05 # Changes on linux
...
def obstacle_collision(ob):
    ...
    dist = math.sqrt((ob_center[0] - car_pos[0])**2 + (ob_center[1] - car_pos[1])**2)
...
def wall_collision():
    ...
    car_top = car_pos[1] + CAR_LENGTH / 2 * math.sin(math.radians(car_angle))
    car_bottom = car_pos[1] - CAR_LENGTH / 2 * math.sin(math.radians(car_angle))
...
def main():
    glutInit()
    glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGB)  # TODO : GLUT_DEPTH
